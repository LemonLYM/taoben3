  ...
  <div>
    <el-row :gutter="30">
      <el-col v-bind="grid">
        <div class="Nav">
          <div class="input">
            <el-input v-model="filterText" placeholder="选择分类" prefix-icon="el-icon-search" style="width: 100%;"
                      clearable/>
          </div>
                  ...
                  <div slot-scope="{ node, data}" class="custom-tree-node" @click.stop="handleNodeClick(data)">
                    ...
                    <span class="el-ic">
                      ...
                      <!--                      />-->
                                        <i
                                          v-if="data.space_id!='0' && (!data.children || data.children == 'undefined') && data.attachment_category_id"
                                          class="el-icon-delete" title="删除分类"
      ...
      <el-col v-bind="grid2" class="colLeft">
        <div v-loading="loading" class="conter">
          <div class="bnt">
            <el-button v-if="params !== '/admin/config/picture'" size="mini" type="primary" class="mb10 mr10"
                       @click="checkPics">使用选中图片
            </el-button>
            <el-upload class="upload-demo mr10 mb15" :action="fileUrl" :on-success="handleSuccess" :headers="myHeaders"
                       :show-file-list="false" multiple>
              <el-button size="mini" type="primary">点击上传</el-button>
            </el-upload>
            <el-button type="success" size="mini" @click.stop="onAdd(0)">添加分类</el-button>
            <el-button type="error" size="mini" class="mr10" :disabled="checkPicList.length===0"
            ...
            <el-select v-model="sleOptions.attachment_category_name" placeholder="图片移动至" class="mb15" size="mini">
              <el-option :label="sleOptions.attachment_category_name" :value="sleOptions.attachment_category_id"
                         style="max-width: 560px;height:200px;overflow: auto;background-color:#fff">
                <el-tree ref="tree2" :data="treeData2" :filter-node-method="filterNode" :props="defaultProps"
                         highlight-current @node-click="handleSelClick"/>
              </el-option>
            </el-select>
          </div>
          <div class="pictrueList acea-row">
            ...
            <div class="conters">
              <div v-for="(item, index) in pictrueList.list" :key="index" class="gridPic">
                <img v-lazy="item.attachment_src" :class="item.isSelect ? 'on': '' "
                     @click="changImage(item, index, pictrueList.list)">
              </div>
            </div>
          </div>
          <div class="block">
            <el-pagination :page-sizes="[12, 20, 40, 60]" :page-size="tableData.limit" :current-page="tableData.page"
                           layout="total, sizes, prev, pager, next, jumper" :total="pictrueList.total"
