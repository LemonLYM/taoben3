...
class Order extends BaseController
{
    ...
    {
        $data = $this->request->params(['total_price','pay_price','total_postage']);
        if($data['total_price'] < 0 || $data['pay_price'] < 0 || $data['total_postage'] < 0)
            return app('json')->fail('金额不可未负数');
        if(!$this->repository->merStatusExists($id,$this->request->merId()))
            return app('json')->fail('订单信息或状态错误');
        $data['pay_postage'] = $data['total_postage'];
